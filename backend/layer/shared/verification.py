...
def _safe_ctx(ctx: dict) -> dict:
    keys = (
        "alert_id alert_type risk_score stage priority status amount "
        "sender_account_id receiver_account_id is_first_transfer "
        "device_match channel sender_age_days receiver_age_days "
        "mule_stage unique_inbound_senders_6h inbound_outbound_ratio scam_type"
    ).split()
    out = {}
    for k in keys:
        v = ctx.get(k)
...
async def _bedrock_finding_streamed(agent: str, ctx: dict, run_id: str) -> dict:
    ...
    try:
        ...
        print(f"[verify] stream ({agent}) failed: {e}")
        parsed = {}
...
def load_run(run_id: str) -> dict | None:
    ...
    return _serialise_run(row, findings.get(run_id, []), streams.get(run_id, []))
