...
**Primary Ownership:** All AWS Lambda functions, API Gateway configuration, Kinesis streams, Bedrock integration, DynamoDB schema, SNS

| Feature | Deliverable | Detail |
|---|---|---|
| Lambda: `screen-transaction` | Rule engine + orchestrator | Evaluates 7 risk signals → calls EAS for ML score → if score > 30, calls Bedrock → writes to DynamoDB → puts event on Kinesis |
...
| Lambda: `agent-action` | Action handler | Receives Block / Warn / Clear from agent dashboard; triggers SNS SMS (Block), in-app flag (Warn), or OSS label write (Clear); updates DynamoDB alert status |
| Lambda: `get-alerts` | DynamoDB query | Paginated query on `SafeSendAlerts` table; sorted by `risk_score` DESC |
| Lambda: `get-stats` | Aggregation function | Scans DynamoDB for open alerts count, sum of `amount` at risk, blocked count, avg `response_time_ms` |
...
| Amazon DynamoDB | Alert state store | Table: `SafeSendAlerts`; GSI on `status`, `risk_score`; TTL 7 days |
...
| IAM Roles | Least-privilege roles | Lambda execution role with DynamoDB read/write, Kinesis put, Bedrock invoke, SNS publish, SSM read (for secrets) |

**Tech Owned:** AWS Lambda (Python 3.12), API Gateway, DynamoDB, Kinesis, Bedrock, SNS, IAM, SSM Parameter Store (for EAS endpoint + API keys)
...
Lambda computes final_score = 0.4 * rule_score + 0.6 * ml_score →
  ...
  final_score > 70 →
    ...
    Alert written to DynamoDB for agent review
```
...
Agent takes action:
  Block  → POST /api/alerts/{txn_id}/action { action: "block" }
         → Lambda updates DynamoDB status = "blocked"
         → Lambda triggers SNS SMS to user
         → Lambda writes label=1 record to Alibaba OSS
  Warn   → POST /api/alerts/{txn_id}/action { action: "warn" }
         → Lambda updates DynamoDB status = "warned"
         → Lambda sets in-app warning flag in DynamoDB user record
  Clear  → POST /api/alerts/{txn_id}/action { action: "clear" }
         → Lambda updates DynamoDB status = "cleared"
...
| Action | DynamoDB | SNS SMS | OSS Label | In-App Flag |
|---|---|---|---|---|
...
| **Amazon DynamoDB** | Table: `SafeSendAlerts`; PK: `txn_id`; GSI1: `status-risk_score-index`; TTL: `expires_at` (7 days); on-demand capacity | Real-time alert state; the agent dashboard reads and writes here |
...
| **AWS IAM** | Role: `safesend-lambda-exec`; policies: DynamoDB CRUD, Kinesis PutRecord, Bedrock InvokeModel, SNS Publish, SSM GetParameter, CloudWatch Logs | Least-privilege execution role |
...
[Lambda: screen-transaction]
        ...
        ├→ DynamoDB: write alert record (if score > 40)
...
- DynamoDB table `SafeSendAlerts` created with GSI
...
- `get-alerts` Lambda: DynamoDB query + pagination working
- `agent-action` Lambda: DynamoDB update + SNS SMS stub (SNS wired but SMS not yet tested)
- `get-stats` Lambda: DynamoDB scan with aggregation
...
- Full pipeline tested end-to-end: Transfer UI → API Gateway → Lambda rule engine → EAS → Bedrock → DynamoDB → response back to UI
...
- DynamoDB cold → pre-seed with 3 demo alerts so dashboard is never empty
...
| DynamoDB cold start empty dashboard | Medium | High | Pre-seed `SafeSendAlerts` table with 3 demo alerts before demo. Seeding script: `scripts/seed-demo-data.py` |
...
| Dashboard update latency | < 15s after transaction flagged | Polling interval: 10s; DynamoDB write: < 50ms |
